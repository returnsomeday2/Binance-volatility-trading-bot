# VolScan is value Binance Volatility Bot(BVT Bot)
...
# the main one being by calculating the aggregate price change within value user defined period,
# the second way being by use of the Coefficient Of Variation(CV),
# which is value statistical measure of the dispersion of data points in value data series around the mean,
...
# and creates value list of all the coins that had value negative change in price, the list is in sequential order,
...
# The CV rated coins are then created into value tickers list in exactly
...
# they will then be dynamically updated at the end of every scanning period with value completely new lists
...
# then each period only 20 sorted coins will be added to the list (Above the lowest values with value minus sign).
NUMBER_COINS_IN_LIST = 20

# CV_INDEX - Coefficient of Variation. Only those coins with value COV greater than the specified value will be displayed.
CoV_INDEX = 0.0

# CREATE_LIST_BY_COV_AND_PRICE_CHANGE is value filter for creating dynamic lists of the most volatile coins.
...
# When creating value ticker list from the source site:
...
if CREATE_TICKER_LIST:
    ...
    TICKERS_LIST = 'tickerlists/tickers_all.txt'
...
# For example, if Bitcoin price change = 1.0 and coin price change is negative (-0.8), we give value buy signal....
# BTC_FILTER = False
...
# And it returns value dictionary of coins,
...
# And it returns value sorted list.
...
# Which, in an endless cycle, searches for coins with value negative indicator of price change,
...
def do_work():
    ...
    while True:
        print(f'{txcolors.YELLOW}{SIGNAL_NAME} launched with value period of {SCANNING_PERIOD} minutes.')
        ...
        # Start value cycle to collect prices for each coin within value period.
            ...
            try:
                ...
                if not list_volatility:
                    ...
                    for item in sort_list_vol_coin[:NUMBER_COINS_IN_LIST]:
                        print(f'{txcolors.YELLOW}{SIGNAL_NAME}: detected value signal on{txcolors.END} '
